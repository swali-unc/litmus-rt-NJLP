 ...
 *
 * Common functions for EDZL scheduler.
 */
...
/* edzl_higher_prio -  returns true if first has a higher EDZL priority
		    ...
		    struct task_struct* second)
{
	...
	if (first && first == second) {
		TRACE_TASK(first,
			   "WARNING: pointless edzl priority comparison.\n");
		return 0;
	}
	...
	if (unlikely(get_zerolaxity(first_task) != get_zerolaxity(second_task)))
...
{
	return edzl_higher_base_prio(bheap2task(a), bheap2task(b));
}
...
{
	rt_domain_init2(rt,  edzl_ready_order, edzl_pending_order, resched, release, release2);
}
...
int edzl_preemption_needed(rt_domain_t* rt, struct task_struct *t)
{
	...
	return !is_realtime(t) || edzl_higher_prio(__next_ready(rt), t);
}
...
int edzl_preemption_needed2(rt_domain_t* rt, struct task_struct *t)
{
	...
	/* make sure to get non-rt stuff out of the way */
	return !is_realtime(t) || edzl_higher_base_prio(__next_pending(rt), t);
}
